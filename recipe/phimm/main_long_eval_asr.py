...
The reference implementation in ``/home/boren/code/verl`` uses the legacy
``ActorRolloutRefWorker.generate_sequences`` path. verl-mirror no longer has
that worker; its supported equivalent is the server-based
:mod:`recipe.phimm.long_asr_rollout` pipeline. This entry point keeps the
reference command and config layout while delegating generation, regrouping,
DTER/EER scoring, and result writing to that implementation.

Usage::

    python3 -m recipe.phimm.main_long_eval_asr \
        --config-name long_eval_mixlang_fy26q2
"""
