...
Delegate each remote job's submission, monitoring, failure recovery, and result retrieval to [verl-asr-run](../verl-asr-run/SKILL.md). This skill owns the cross-node work queue, recovery of that queue, benchmark selection, baseline discipline, and workbook assembly. Read the [shared remote execution contract](../references/remote-execution.md) before scheduling.

For several explicitly requested models, maintain a separate matrix/workbook per model and assign disjoint pools or process models sequentially. A child job must not restart this workflow. A report-only request uses existing artifacts and does not launch evaluations. The fixed 2607 mixed/AA suite belongs to [eval-mix-openml-aa-report](../eval-mix-openml-aa-report/SKILL.md), not this suite.
...
|---|---|---|---|
| `inhouse_dter` | `recipe/phimm/config/eval_v2609/long_eval_inhouse_all_seg30.yaml` | micro-DTER | `model.path` in the config and its reference result directory. |
| `digits_enus` | `recipe/phimm/config/eval_v2609/eval_digits_enus.yaml` | Digit CER and WER | Required by default; baseline is `actor_rollout_ref.model.path`. |
| `openasr_ml` | `recipe/phimm/config/eval_v2609/eval_openasr_ml_verb.yaml` | WER / `p_err` | `actor_rollout_ref.model.path` in the config. |
| `mixlang` | `recipe/phimm/config/eval_v2609/long_eval_mixlang_fy26q2_zh_seg.yaml` | DTER / TER | `model.path` in the config and its reference result directory. |
| `digits_tier1` | `recipe/phimm/config/eval_v2609/eval_digits_tier1.yaml` | Digit CER and WER | Only include when `--include-digits-tier1`; baseline is `actor_rollout_ref.model.path`. |

The reference checkpoint and candidate must use the same config, data, locale, scoring backend, and segment settings. In particular, do not compare MixLang's zh-CN TER to an unrelated DTER result. An explicitly requested subset is a partial report, not a complete-suite result; apply the same subset to every checkpoint.
...
2. Build a work matrix with one row for every requested checkpoint and required candidate benchmark, including `digits_enus`. Add a matching reference row whenever verified compatible reference outputs are unavailable. Digits have no embedded baseline; the other embedded values are historical and require provenance verification before reuse. Give every row a unique experiment name and output path below `<artifact-root>/<checkpoint>/<benchmark>/<candidate|reference>/`. On recovery, reconcile matching live jobs and complete durable artifacts before submission; package existing outputs missing metadata instead of rerunning them. Never submit the same row twice.
3. Reconstruct occupancy across eligible Ready `verl-*` pools or the supplied `--node` allowlist using the shared occupancy/resource checks. Unknown occupancy blocks allocation. If no eligible pool is free, keep rows queued rather than overcommitting or moving outside the allowlist.
4. Apply the shared topology and sync procedure to each selected pool. Record its verified `EVAL_NNODES` independently and verify/sync the current workspace before its first submission.
...
7. Use embedded values only when their provenance is verified against the resolved config reference; otherwise pass explicit matching reference results. Candidate and reference rows for the same benchmark may run on different pools, but both must use the same committed config, data, locale, scoring backend, and segment settings.
...
**Collecting the baseline result.** The builder falls back to the following historical embedded values when `--<bench>-baseline` is omitted. This is a parser default, not evidence of compatibility with the current config:
...
An explicit `--<bench>-baseline` source overrides the embedded values. The current configs reference an MTP checkpoint at step 2000, while the embedded labels say `2609v1`; do not equate them without provenance. When compatibility is unverified, run the same config with its reference model through `verl-asr-run` or reuse matching retained outputs, then pass the explicit baseline source and actual reference path/label to the builder. Digits always need an explicit baseline source. Collect candidate results with the candidate model path.
...
/home/boren/.virtualenvs/openai/bin/python \
  .github/skills/eval-2609-benchmark-report/scripts/build_2609_report.py \
   ...
   --reference-model-path '<resolved-config-reference-path>' \
   --baseline-label '<verified-reference-label>' \
  ...
  --inhouse-dter  az://orngwus2cresco/.../cand/inhouse_2605_all_seg30/ \
   --inhouse-dter-baseline az://orngwus2cresco/.../ref/inhouse_2605_all_seg30/ \
  ...
  --mixlang           az://orngwus2cresco/.../cand/long_audio_mixlang_fy26q2_zh_seg/ \
   --mixlang-baseline az://orngwus2cresco/.../ref/long_audio_mixlang_fy26q2_zh_seg/ \
...
Omit a baseline flag only after verifying the corresponding embedded provenance. Always add `--digits-enus <cand>` with `--digits-enus-baseline <base>` for the complete suite. Add `--digits-tier1 <cand>` with `--digits-tier1-baseline <base>` only when requested. Only benchmarks whose candidate source is supplied get a sheet. The builder accepts one reference path/label for the checkpoint workbook; if benchmarks resolve to different references, correct each sheet/summary row and sidecar with its actual per-benchmark provenance before merging.
...
The current chart helper has a known regression covered by
[`test_merged_summary_chart_preserves_multiple_metrics_per_benchmark`](../../../tests/skills/test_build_2609_report.py):
merged charts group by benchmark alone and can collapse digits CER/WER. Run that
test before delivery and do not claim chart validation passed while it fails.
The checks below remain requirements, not guarantees made by the builder.

Before delivering the workbook, verify all of the following:

- Every required benchmark has a successful candidate job (or verified complete durable artifacts during recovery) and a provenance-verified matching baseline.
...
- No unverified historical embedded baseline was silently used for a different config reference.
