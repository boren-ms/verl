...
9. The script also emits the `summary` sheet with one row per included benchmark metric: the aggregate baseline and candidate values, the overall delta, and the source config. Summary delta columns use the same red→white→green color scale centered at 0 as benchmark delta columns, so regressions are red and gains are green. Digits rows appear only for the explicitly requested digits evaluations.
