...
description: "Evaluate one or more checkpoint steps of the same 2607 ASR model on the standard in-house DTER, OpenASR-ML, and MixLang benchmarks remotely and create one baseline-aware multi-step Excel report per model. Use when: evaluate 2607 benchmarks, run 2607 model evaluation, build a consolidated 2607 benchmark report, compare checkpoints across steps, or compare a 2607 model against its reference model."
...
3. For every requested checkpoint and required config, run a candidate evaluation remotely through `verl-asr-run`, using a unique experiment name and output path below `<artifact-root>/<checkpoint>/<benchmark>/candidate/`. Always pass `trainer.nnodes=${EVAL_NNODES}` together with the candidate model path and output/experiment overrides. Configure the evaluation so detailed decoding outputs are written or uploaded to that remote blob path, never left only on the Brix node.
...
6. Before releasing a node or creating the workbook, retain the raw information for every candidate run and every explicitly executed reference run under `<artifact-root>/<checkpoint>/<benchmark>/<candidate|reference>/`:
...
9. Build the single deliverable workbook with [scripts/merge_2607_reports.py](./scripts/merge_2607_reports.py), even when only one checkpoint is requested. Pass each staged workbook as `--report <checkpoint-label>=<staged-xlsx>` in requested order and write the result to `--out` or `tmp/eval_2607_reports/<model-label>.xlsx`. The consolidated workbook contains:
...
/home/boren/.virtualenvs/openai/bin/python \
   ...
   --report "step840=tmp/eval_2607_reports/remax_2607/.staging/step840.xlsx" \
   --out tmp/eval_2607_reports/remax_2607.xlsx
```

Keep each checkpoint's benchmark sheet self-contained so its baseline, formulas, and provenance remain auditable. Use the consolidated `summary` sheet for cross-step sorting and comparison. Once the final workbook passes the quality gates, remove `tmp/eval_2607_reports/remax_2607/.staging/`.

### Repair existing workbooks

Use [scripts/repair_2607_avg_formulas.py](./scripts/repair_2607_avg_formulas.py) to update an existing consolidated or legacy 2607 workbook in place. The utility replaces stored numeric group and overall averages with arithmetic Excel `AVERAGE(...)` formulas, refreshes summary aggregates from dataset rows, and enables full automatic recalculation.
