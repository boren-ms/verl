...
@pytest.mark.parametrize(
    ...
    [
        ("The code is AB020X.", "The code is AB029X.", (1, 1, 1, 1, 3)),
        ("Call 020-394-1446 today.", "Call 020-394-1447 today.", (1.0 / 3.0, 1, 3, 1, 10)),
        ("Order #12 ships in 3 days.", "Order #12 ships in 4 days.", (0.5, 1, 2, 1, 3)),
        ("Use zero two zero for access.", "Use zero two nine for access.", (1.0 / 3.0, 1, 3, 1, 3)),
        ("IDs 02 and 94 are active.", "IDs 02 and 95 are active.", (0.5, 1, 2, 1, 4)),
    ],
)
def test_score_digits_extracts_digits_from_strings(reference, hypothesis, expected):
    wer, n_err, n_ref, nc_err, nc_ref = expected
...
@pytest.mark.parametrize(
    ("reference", "hypothesis", "wer", "n_err"),
    [
        ("0203", "zero two zero", 3.0, 3),
        ("0203", "02039", 1.0, 1),
        ("0203", "0293", 1.0, 1),
    ],
)
def test_score_digits_reports_partial_sequence_errors(reference, hypothesis, wer, n_err):
    assert score_digits(reference, hypothesis) == {
        "wer": pytest.approx(wer),
        "n_err": n_err,
        "n_ref": 1,
...
def test_eval_score_matches_spoken_digit_words():
    ...
    assert result["wer"] == 4.0
    assert result["cer"] == 0.0
    assert result["n_err"] == 4
    assert result["n_ref"] == 1
...
def test_eval_score_reports_digit_substitution():
    ...
    assert result["wer"] == 3.0
    assert result["score"] == pytest.approx(2.0 / 3.0)
    assert result["n_err"] == 3
    assert result["n_ref"] == 1
