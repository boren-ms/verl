...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # LoRA normally uses the actor without adapters as the reference. Privileged inputs
        # require a standalone reference worker so it can receive a different prompt.
        self.ref_in_actor = (
            config.actor_rollout_ref.model.get("lora_rank", 0) > 0
            and not config.actor_rollout_ref.ref.get("use_privileged_inputs", False)
        )
